...
sub PrintStatuses{
	...
	my $filter = " 1=1 ";
	$filter .= " AND r.user_id=".$dbh->quote(param('user_id')) if param('user_id');
	$filter .= " AND r.problem_id=".$dbh->quote(param('problem_id')) if param('problem_id');
	$filter .= " AND p.contest_id=".$dbh->quote(param('contest_id')) if param('contest_id');
	$filter .= " AND r.run_id<=".$dbh->quote(param('to_run_id')) if param('to_run_id');
